class LandingController {
    ...
    showPlanetModal(planetElement) {
         const planetClasses = ['club', 'mission', 'goals', 'roadmap', 'projects'];
    const planetIndex = Array.from(document.querySelectorAll('.planet-wrapper')).indexOf(planetElement);
    const planetClass = planetClasses[planetIndex];
    ...
    if (!planetData || !planetData.title) {
        console.log('Planet data not found for:', planetClass);
        return;
    }
        if (!planetClass) {
            console.log('No planet class found');
